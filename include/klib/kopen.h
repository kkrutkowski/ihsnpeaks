...
#    define __err_connect(func) \
    ...
    if ((ai_err = getaddrinfo(host, port, &hints, &res)) != 0) {
        fprintf(stderr, "can't resolve %s:%s: %s\n", host, port, gai_strerror(ai_err));
        return -1;
    }
...
static int kftp_get_response(ftpaux_t *aux) {
    ...
    while (read(aux->ctrl_fd, &c, 1)) {  // FIXME: this is *VERY BAD* for unbuffered I/O
        ...
        aux->response[n++] = c;
        if (c == '\n') {
            if (n >= 4 && isdigit(aux->response[0]) && isdigit(aux->response[1]) && isdigit(aux->response[2]) && aux->response[3] != '-') break;
            n = 0;
            continue;
        }
    }
...
static char **cmd2argv(const char *cmd) {
    ...
    for (i = beg + 1, argc = 0; i < end; ++i)
        if (isspace(cmd[i]) && !isspace(cmd[i - 1])) ++argc;
...
int main(int argc, char *argv[]) {
    ...
    do {
        if ((l = fread(buf, 1, BUF_SIZE, fp)) != 0) fwrite(buf, 1, l, stdout);
    } while (l == BUF_SIZE);
    fclose(fp);
    kclose(x);
    return 0;
