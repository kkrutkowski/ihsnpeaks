...
typedef union {
    uint64_t data; // The raw 64-bit representation
    struct {
        uint8_t key;        // 16-bit key
        uint8_t empty;        // 16-bit key
        uint16_t placeholder; // 16-bit placeholder
        float value;         // 32-bit floating-point value
    } parts;
} kvpair;
...
static inline uint8_t extract_key(uint64_t value) { return (uint8_t) value; } // Assuming lowest 16 bits only on little endian

void bsort64_8(uint64_t** array, size_t n, uint64_t** aux_buffer, size_t* aux_buffer_size, size_t* indices) {
    ...
    for (size_t i = 1; i < 256; i++) {indices[i] += indices[i - 1];}

    // Rearrange elements into the aux_buffer based on indices (placement phase)
    for (size_t i = 0; i < n; i++) {
        uint8_t index_tmp = extract_key((*array)[i]);
