...
def detect_stdflag(cc: str, cwd: Path) -> str:
    ...
    return "-std=gnu23" if probe.returncode == 0 else "-std=gnu11"
...
def main(argv: list[str]) -> int:
    ...
    objcopy = shutil.which(args.objcopy) or args.objcopy
    strip = shutil.which(args.strip) or args.strip
    ...
    variant_objects = [compile_variant(root, build_root, cc, objcopy, stdflag, base_cflags, variant) for variant in variants]
